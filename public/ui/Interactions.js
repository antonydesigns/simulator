...
function addNode(type, wx, wy) {
  ...
  if (type === 'load') {
    ...
    node = { id: uid(), type, x: wx, y: wy, shortId: shortId(type), label: '', mw: 0, rating: 100, inertia: 5, droop: 0.04, baselineContract: 0, fcrHeadroom: 10, bidPrice: 50, committedMW: 100, mode: 'balancing', turbineTimeConstant: 1, rampDownTC: 0.3, agcOffset: 0, tripped: false, freqTimer: 0 };
  ...
  if (meta && e.shiftKey && e.key === 'V' && state.clipboard && state.clipboard.nodes && state.clipboard.nodes.length === 1 && state.selectedNodeIds.size > 0) {
    ...
    for (const t of targets) {
      if (src.type === 'generator') {
        ...
        t.fcrHeadroom = src.fcrHeadroom; t.bidPrice = src.bidPrice; t.committedMW = src.committedMW;
